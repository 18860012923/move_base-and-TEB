                                        ...
                                        pf_kdtree_node_t *node, int key[], double value)
{
  ...
  if (node == NULL)
  {
    //ROS_INFO_STREAM("\033[1;32mnode == NULL)!!!!!!!!!\033[0m");
...
void pf_kdtree_insert(pf_kdtree_t *self, pf_vector_t pose, double value)
{
  ...
  //ROS_INFO_STREAM("\033[1;32mpf_kdtree_insert_node!!!!!!!!!\033[0m");
...
// Pose hypothesis
// 给定初始化Pose值的数据结构：权重和概率参数(均值和协方差)
typedef struct
{
 ...
 * with old rosbags that have trailing slashes for their frames
 此功能仅适用于让整个代码与旧的 rosbags 一起使用，这些旧 rosbags 的帧有斜杠
 */
inline
...
class AmclNode
{
  ...
  private:
    ...
    void camera_pose_setinit(pf_sample_set_t* set,int max_samples);
    void cameraReceived(const geometry_msgs::PoseArrayConstPtr& msg);
    void handleCameraReceived(const geometry_msgs::PoseArray& msg);
    bool oritation_diff(const geometry_msgs::Quaternion& corientation,const geometry_msgs::Quaternion& rotation);
    double pose_diff(const geometry_msgs::Point& cposition,const geometry_msgs::Vector3& translation);
    ...
    //相机获得的posearray
    pf_sample_set_t* camera_pose_set;
    bool first_camera_only_;
  ...
  //beam skip related params
    ...
    double angle_threshold_,pose_threshold_;
    ...
    ros::Time last_camera_received_ts_;
    ros::Duration laser_check_interval_;
    ros::Duration camera_check_interval_;
    void checkLaserReceived(const ros::TimerEvent& event);
    void checkCameraReceived(const ros::TimerEvent& event);
};
...
main(int argc, char** argv)
{
  ...
  std::cout << "amcl_node_ptr" << std::endl;
...
AmclNode::AmclNode() :
        ...
        if_robot_pose_get_(false),
        first_camera_only_(false)
{
  ROS_INFO_STREAM("\033[1;32mAmclNode::AmclNode!!!!!!!!!\033[0m");
  ...
  private_nh_.param("angle_threshold", angle_threshold_, 0.78);
  private_nh_.param("pose_threshold", pose_threshold_, 2.3);
  ...
  private_nh_.param("if_use_signs", if_use_signs_, false);
  // For diagnostics
  ...
  std::cout << "updatePoseFromServer" << std::endl;
  ...
  if(if_use_signs_) {
    camera_pose_sub_ = nh_.subscribe("camera_cloud", 1, &AmclNode::cameraReceived, this);
    camera_pose_set = (pf_sample_set_t*)calloc(1,sizeof(pf_sample_set_t));
    ROS_INFO("Subscribed to camera_cloud topic.");
  }
  ...
  // 0.5s timer to warn on lack of receipt of camera, #5209
  camera_check_interval_ = ros::Duration(4.5);
  check_camera_timer_ = nh_.createTimer(camera_check_interval_, 
                                       boost::bind(&AmclNode::checkCameraReceived, this, _1));
...
{
  ROS_INFO_STREAM("\033[1;32mreconfigureCB!!!!!!!!!\033[0m");
...
{
  //ROS_INFO_STREAM("\033[1;32msavePoseToServer!!!!!!!!!\033[0m");
...
{
  ROS_INFO_STREAM("\033[1;32mupdatePoseFromServer!!!!!!!!!\033[0m");
  init_pose_[0] = 0.0;
  init_pose_[1] = 0.0;
  init_pose_[2] = 0.0;
  init_cov_[0] = 0.5 * 0.5;
...
{
  ROS_INFO_STREAM("\033[1;32mcheckLaserReceived!!!!!!!!!\033[0m");
...
void
AmclNode::checkCameraReceived(const ros::TimerEvent& event)
{
  ros::Duration d = ros::Time::now() - last_camera_received_ts_;
  if(d > camera_check_interval_)
  {
    //ROS_INFO_STREAM("\033[1;32mif_robot_pose_get_ = false;!!!!!!!!!\033[0m");
    if_robot_pose_get_ = false;
  }
}
...
AmclNode::requestMap()
{
  ROS_INFO_STREAM("\033[1;32mrequestMap!!!!!!!!!\033[0m");
...
{
  ROS_INFO_STREAM("\033[1;32mmapReceived!!!!!!!!!\033[0m");
...
void
AmclNode::cameraReceived(const geometry_msgs::PoseArrayConstPtr& msg)
{
  handleCameraReceived(*msg);
}

void 
AmclNode::camera_pose_setinit(pf_sample_set_t* set,int max_samples)
{
  set->sample_count = max_samples;
  //ROS_INFO_STREAM("\033[1;32mset->samples calloc!!!!!!!!!\033[0m");
  set->samples = (pf_sample_t*)calloc(max_samples, sizeof(pf_sample_t));

  // HACK: is 3 times max_samples enough?
  //ROS_INFO_STREAM("\033[1;32mset->kdtree calloc!!!!!!!!!\033[0m");
  set->kdtree = pf_kdtree_alloc(3 * max_samples);

  set->cluster_count = 0;
  set->cluster_max_count = max_samples;
  //ROS_INFO_STREAM("\033[1;32mset->clusters calloc!!!!!!!!!\033[0m");
...
void
AmclNode::handleCameraReceived(const geometry_msgs::PoseArray& msg)
{
  last_camera_received_ts_ = ros::Time::now();
  boost::recursive_mutex::scoped_lock car(configuration_mutex_);
  ROS_INFO_STREAM("\033[1;32minto handleCameraReceived!!!!!!!!!\033[0m");
  try{
    geometry_msgs::TransformStamped curr_rob = tf_->lookupTransform(global_frame_id_,base_frame_id_,ros::Time(0));
    //ROS_INFO_STREAM("\033[1;32minto handleCameraReceived!!!!!!!!!\033[0m");
    
    double total = 0;
    
    if(first_camera_only_)
    {
      //ROS_INFO_STREAM("\033[1;32minto free!!!!!!!!!\033[0m");
      free(camera_pose_set->clusters);
      free(camera_pose_set->kdtree->nodes);
      free(camera_pose_set->kdtree);
      free(camera_pose_set->samples);
    }

    camera_pose_setinit(camera_pose_set,msg.poses.size());
    first_camera_only_ = true;

    int k = 0;
    assert(msg.poses.size() > 0);
    pf_sample_t *sample;
    for(int i = 0 ;i < msg.poses.size();i++)
    {
      // 先直接去掉所有与当前base_link到map变换情况下方向和位置相差过大的点
      sample = camera_pose_set->samples + i;
      sample->pose.v[0] = msg.poses[i].position.x;
      sample->pose.v[1] = msg.poses[i].position.y;
      sample->pose.v[2] = tf2::getYaw(msg.poses[i].orientation);
      if(oritation_diff(msg.poses[i].orientation,curr_rob.transform.rotation))
      {
        // 对符合要求的粒子，加入到camera_pose_set中，权重按照到到机器人欧式距离计算
        sample->weight = 1.0 / pose_diff(msg.poses[i].position,curr_rob.transform.translation);
      }
      else
      {
        std::cout << "cut out index " << i << " pose." << std::endl;
        sample->weight = 1.0 / INT_MAX;
      }
      total += sample->weight;
      pf_kdtree_insert(camera_pose_set->kdtree, sample->pose, sample->weight);
    }
    for (int i = 0; i < camera_pose_set->sample_count; i++)
    {  
      sample = camera_pose_set->samples + i;
      sample->weight /= total;
    }
    if(camera_pose_set->sample_count != 0) if_robot_pose_get_ = true;
    else if_robot_pose_get_ = false;
  }
  catch(tf2::TransformException e)
  {
      ROS_WARN("Failed to transform initial pose in time (%s),carmera msg has not Received.", e.what());
  }
  catch (...) {
...
bool
AmclNode::oritation_diff(const geometry_msgs::Quaternion& corientation,const geometry_msgs::Quaternion& rotation)
{
  double inner_product = corientation.x * rotation.x +
                         corientation.y * rotation.y +
                         corientation.z * rotation.z +
                         corientation.w * rotation.w;
  if(inner_product > angle_threshold_) return true;
  else return false;
}

double
AmclNode::pose_diff(const geometry_msgs::Point& cposition,const geometry_msgs::Vector3& translation)
{
  double distance = sqrt(
    pow(cposition.x - translation.x,2) +
    pow(cposition.y - translation.y,2) +
    pow(cposition.z - translation.z,2)
  );
  if(distance > pose_threshold_) return INT_MAX;
...
{
  ROS_INFO_STREAM("\033[1;32mhandleMapMessage!!!!!!!!!\033[0m");
  boost::recursive_mutex::scoped_lock cfl(configuration_mutex_);
 ...
 * 其中100表示障碍物occupy，0表示未占用free，-1表示未知状态unknown
 */
map_t*
AmclNode::convertMap( const nav_msgs::OccupancyGrid& map_msg )
{
  ROS_INFO_STREAM("\033[1;32mconvertMap!!!!!!!!!\033[0m");
  map_t* map = map_alloc();
  ROS_ASSERT(map);
...
{
  //ROS_INFO_STREAM("\033[1;32minto laserReceived!!!!!!!!!\033[0m");
  ...
  //ROS_INFO_STREAM("\033[1;32minto frame_to_laser_!!!!!!!!!\033[0m");
  ...
  if(lasers_update_[laser_index])
  {
    ...
    {
      //std::cout << if_use_signs_ << " " << if_robot_pose_get_ << std::endl;
      if(if_use_signs_ && if_robot_pose_get_) 
      {
        ROS_INFO_STREAM("\033[1;32minto new_pf_update_resample!!!!!!!!!\033[0m");
        printf("camera_pose_set->sample_count:%d\n",camera_pose_set->sample_count);
        new_pf_update_resample(pf_,camera_pose_set);
      }
      else 
      {
        ROS_INFO_STREAM("\033[1;32minto pf_update_resample!!!!!!!!!\033[0m");
...
{
  ROS_INFO_STREAM("\033[1;32mhandleInitialPoseMessage!!!!!!!!!\033[0m");
